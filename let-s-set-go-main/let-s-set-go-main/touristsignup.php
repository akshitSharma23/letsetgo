...
<div class="container" style="width:50%;align:center; margin-top:5%;border:2px solid lightgray; border-radius:50px;background-color:#162558;">
  ...
  <div class="form-group">
	...
	<div class="form-group">
      <label for="pwd" id="color">Password(min length 7):</label>
...
if(isset($_POST['sbutton']))
{
 ...
 $query = mysqli_query($con, "SELECT * from tourist where email='$email'");
...
{
	$user_reg_query="INSERT INTO tourist(name,email,password) values ('$name','$email','$password')" or die($user_reg_query);
    
	
	if(mysqli_query($con,$user_reg_query))
	{
		echo "<script>alert('$name User Created Sucessfully ');</script>";
